...
router.post('/phone', function (req, res) {
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    if (req.body.username != "" && req.body.password != "" && req.body.username != null && req.body.password != null) {
      ...
      db.query('SELECT secure_login_ID , secure_login_password from secure_login where ? ', paremeters, function (err, rows, fields) {
        if (err) {
          res.statusCode = 200;
          return res.json({
            respond: "Missing Fields",
...
router.put('/phone', function (req, res) {
  if (req.body.constructor === Object && Object.keys(req.body).length === 0) {
    ...
    if (req.body.app_token != "" && req.body.app_token != null && req.body.username != "" && req.body.username != null) {
      ...
      db.query('SELECT secure_login_ID from secure_login where ? AND ?', [paremeters1, paremeters2], function (err, rows, fields) {
        if (err) {
          res.statusCode = 200;
          return res.json({
            respond: "Database error",
